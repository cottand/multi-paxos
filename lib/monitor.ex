...
defmodule Monitor do
  ...
  def transactions(state, v), do: Map.put(state, :transactions, v)

  def commanders_spawned(state, i, v),
    do: Map.put(state, :commanders_spawned, Map.put(state.commanders_spawned, i, v))

  def commanders_finished(state, i, v),
    do: Map.put(state, :commanders_finished, Map.put(state.commanders_finished, i, v))

  def scouts_spawned(state, i, v),
    do: Map.put(state, :scouts_spawned, Map.put(state.scouts_spawned, i, v))

  def scouts_finished(state, i, v),
    do: Map.put(state, :scouts_finished, Map.put(state.scouts_finished, i, v))

  def start_print_timeout(duration), do: Process.send_after(self(), {:PRINT}, duration)
  ...
  def next(config, state) do
    receive do
      {:DB_UPDATE, db, seqnum, transaction} ->
        ...
        if seqnum != done + 1,
          do: Util.halt("  ** error db #{db}: seq #{seqnum} expecting #{done + 1}")
