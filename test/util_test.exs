...
defmodule UtilTest do
  ...
  test "ballots from diff PID with different nums are compared properly" do
    b1 = {0, 1, spawn(fn -> nil end)}
    b2 = {1, 2, spawn(fn -> nil end)}
    assert Util.ballot_greater?(b2, b1)
  ...
  test "ballots from diff PID with same number can be compared consistently" do
    pid1 = spawn(fn -> nil end)
    pid2 = spawn(fn -> nil end)
    assert comapre_numerical({0, 1, pid1},{0, 2, pid2}) == ballot_greater?({0, pid1},{0, pid2})
    assert comapre_numerical({0, 2, pid2},{0, 1, pid1}) == ballot_greater?({0, pid2},{0, pid1})
    b1 = ballot_as_num {0, 1, pid1}
    b2 = ballot_as_num {0, 2, pid2}
    greater = b1 > b2
    if greater do
      assert not (b2 > b1)
    else
